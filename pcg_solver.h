...
{
   if (factor.n!=rhs.size()){
	   cout<<"assert shit";
	   return;
   }
   if (factor.n!=result.size()){
   	   cout<<"assert shit";
   	   return;
      }
   result=rhs;
...
struct PCGSolver
{
   ...
   void add_scaled(float alpha, const std::vector<float> &x, std::vector<float> &y){
	   for (unsigned int i=0;i<x.size();i++){
		   y.at(i)=alpha*x.at(i)+y.at(i);
	   }
   }

   float abs_max(const std::vector<float> &x){
	   double max=0;
